...
- Breadcrumb "new run"; header "New run". Stacked cards (white, border `#e2e5ea`, radius 8, padding 16–20px):
  ...
  - **Port mapping card (optional)**: dynamic row list — old port input → new port input, ✕ per row, "+ add pairing" button. Nodes are implied by the devices card, so a row is just two ports. Ports normalized via `normalize_port`; duplicate *old* port rejected inline; several old ports → one new port (the `ae0` case) explicitly allowed. **Zero rows is valid** and creates a mapping-less run.
