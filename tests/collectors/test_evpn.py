...
def test_mac_count_merges_domains_of_one_instance(rpc_fixture):
    ...
    # Nahravka 2026-09-03 (task 5c, post-migration z .5): et-0/0/8.4094 se
    # naucil MAC ve stejnem VL-4094 vedle ae0.4094 - realny stav po presunu
    # sluzeb na et-0/0/8, ne regrese parseru.
    assert set(pop1["interfaces"]) == {"ae0.14", "ae0.15", "ae0.4094", "et-0/0/8.4094"}
...
def test_auto_generated_esi_does_not_hide_real_esi(rpc_fixture):
    ...
    #
    # Nahravka 2026-09-03 (task 5c) ukazala tri realne ESI misto jednoho -
...
def test_instance_irb_carries_l3_context(rpc_fixture):
    ...
    pop1 = result["EVPN-VLAN-AWARE-POP1"]
    # Nahravka 2026-09-03 (task 5c, post-migration z .5): irb.4094/MGMT
    # pribyl vedle irb.14/irb.15 - realny stav po presunu sluzeb, ne
    # regrese parseru (viz i test_auto_generated_esi_does_not_hide_real_esi).
    assert pop1["irb_interfaces"]["total"] == 3
    ...
    assert len(pop1["irb_interfaces"]["entries"]) == 3
    ...
    assert {"name": "irb.4094", "status": "Up", "l3_context": "MGMT"} in (
        pop1["irb_interfaces"]["entries"]
    )
...
def test_instance_esis_exclude_autogenerated(rpc_fixture):
    ...
    #
    # Nahravka 2026-09-03 (task 5c) ukazala tri realne ESI misto jednoho -
    # ae0.4094 pribyl vedle ae0.14/ae0.15 po presunu sluzeb na .5.
    result = EvpnInstanceCollector().parse(
        rpc_fixture("junos-evo", "evpn_instance"), "junos-evo"
    )
    ...
    assert esis == {
        ...
        "00:11:12:13:14:40:94:00:00:00": "Resolved by IFL ae0.4094",
    }
